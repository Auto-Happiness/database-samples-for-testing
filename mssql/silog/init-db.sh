...
# Variables
DB_NAME="tapalord"
# SA_PASSWORD is provided via environment
...
if [ $ready -eq 1 ]; then
    ...
    /opt/mssql-tools18/bin/sqlcmd -S mssql -U sa -P "$SA_PASSWORD" -C -Q "IF NOT EXISTS (SELECT name FROM sys.databases WHERE name = '$DB_NAME') CREATE DATABASE $DB_NAME;"
    
    echo "Running initialization SQL script..."
    /opt/mssql-tools18/bin/sqlcmd -S mssql -U sa -P "$SA_PASSWORD" -d "$DB_NAME" -C -i "/docker-entrypoint-initdb.d/tapalord.sql"
